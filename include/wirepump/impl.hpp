...
template <typename Stream, typename T>
struct write_impl {
    static auto write(Stream & stream, T & v) -> coroutines::sync_task<void> {
        if constexpr (std::same_as<uint8_t, T>) {
...
using read_result = decltype(wirepump::read_impl<Stream, uint8_t>::read(std::declval<Stream &>(), std::declval<uint8_t &>()));

template <typename Stream>
using write_result = decltype(wirepump::write_impl<Stream, uint8_t>::write(std::declval<Stream &>(), std::declval<uint8_t const &>()));
